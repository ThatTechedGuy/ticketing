...
const init = async () => {
  ...
  if (!process.env.STRIPE_KEY) {
    throw Error("STRIPE_KEY not defined");
  }
  ...
  try {
    ...
    app.listen(3000, () =>
      console.log(
        "Payments service has started running. Listening on port 3000!"
      )
    );
  } catch (err) {
    console.error("Failed to connect to payments-mongo sevice:", err);
  }
};
